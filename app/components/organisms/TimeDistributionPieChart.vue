...
const segments = computed(() => {
  const regPct = props.distribution.regularHours / total.value
  const overPct = props.distribution.overtimeHours / total.value
  const absPct = props.distribution.absenceHours / total.value

  const regDash = regPct * circumference.value
  const overDash = overPct * circumference.value
  const absDash = absPct * circumference.value

  return {
    regular: {
      dasharray: `${regDash}, ${circumference.value}`,
      dashoffset: 0
    },
    overtime: {
      dasharray: `${overDash}, ${circumference.value}`,
      dashoffset: -regDash
    },
    absence: {
      dasharray: `${absDash}, ${circumference.value}`,
      dashoffset: -(regDash + overDash)
    }
  }
})
</script>
...
<template>
  <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl shadow-sm w-full flex flex-col items-center justify-center relative min-h-80" :class="rootClass">
    ...
    <div class="relative flex items-center justify-center" :class="chartContainerClass">
      <svg class="w-full h-full -rotate-90 transform" viewBox="0 0 140 140">
        <circle cx="70" cy="70" :r="radius" class="stroke-gray-100 dark:stroke-gray-800" stroke-width="12" fill="none" />

        <circle cx="70" cy="70" :r="radius" class="stroke-indigo-500 transition-all duration-1000 ease-out"
                stroke-width="12" fill="none"
                :stroke-dasharray="segments.regular.dasharray"
                :stroke-dashoffset="segments.regular.dashoffset"
                stroke-linecap="round" />

        <circle cx="70" cy="70" :r="radius" class="stroke-amber-500 transition-all duration-1000 ease-out"
                stroke-width="12" fill="none"
                :stroke-dasharray="segments.overtime.dasharray"
                :stroke-dashoffset="segments.overtime.dashoffset"
                stroke-linecap="round" />

        <circle cx="70" cy="70" :r="radius" class="stroke-red-500 transition-all duration-1000 ease-out"
                stroke-width="12" fill="none"
      ...
      <div class="absolute flex flex-col items-center justify-center text-center">
        <span class="text-2xl font-black text-gray-900 dark:text-white">
            {{ formatDecimalHours(distribution.totalExpectedHours, false) }}
        </span>
    ...
    <div class="flex items-center justify-between w-full border-t border-gray-100 dark:border-gray-800" :class="legendContainerClass">
      <div class="flex flex-col items-center text-center px-2">
        ...
        <span class="text-sm font-bold text-gray-900 dark:text-white">{{ formatDecimalHours(distribution.regularHours, false) }}</span>
      ...
      <div class="flex flex-col items-center text-center px-2">
        ...
        <span class="text-sm font-bold text-amber-600">{{ formatDecimalHours(distribution.overtimeHours, false) }}</span>
      ...
      <div class="flex flex-col items-center text-center px-2">
        ...
        <span class="text-sm font-bold text-red-600">{{ formatDecimalHours(distribution.absenceHours, false) }}</span>
