...
async function handleGenerate() {
    ...
    if (success) {
        showMessage(t.value.settings.successGenerate, 'success')
    } else {
        showMessage(t.value.settings.errorGenerate, 'error')
    }
...
<template>
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-3xl p-8 shadow-sm">
        ...
        <div class="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-2xl border border-gray-100 dark:border-gray-800">
          ...
          <div v-if="settingsStore.isKeyNewlyGenerated" class="mb-3 px-3 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 text-xs font-bold rounded-lg flex items-center gap-2">
          ...
          <div class="flex flex-col sm:flex-row gap-3">
            <div class="relative flex-1">
              <input
                  ...
                  :class="{'text-gray-400 italic': !settingsStore.currentApiKey, 'border-emerald-500 ring-1 ring-emerald-500': settingsStore.isKeyNewlyGenerated}"
              />

              <button
                  v-if="settingsStore.currentApiKey && settingsStore.isKeyNewlyGenerated"
        ...
        <div v-if="settingsStore.currentApiKey" class="mt-6 flex items-start gap-3 p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-xl">
            ...
            <div>
                <h4 class="text-sm font-bold text-amber-800 dark:text-amber-500">{{ t.settings?.warningTitle }}</h4>
        ...
        <ConfirmModal
                ...
                :title="t.settings?.confirmGenerateTitle"
                :message="t.settings?.confirmGenerateMsg"
