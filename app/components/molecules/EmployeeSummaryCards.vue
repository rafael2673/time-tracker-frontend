...
<template>
  <div class="grid grid-cols-1 gap-6">
    <div class="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-6 shadow-sm flex flex-col justify-between transition-all relative overflow-hidden group" :class="props.isLoading ? 'opacity-60' : ''">
      <div class="absolute -right-4 -top-4 w-32 h-32 bg-blue-50 dark:bg-blue-900/10 rounded-full blur-2xl group-hover:bg-blue-100 dark:group-hover:bg-blue-900/20 transition-colors pointer-events-none"></div>
      <div class="flex items-start justify-between mb-4 relative z-10">
        ...
        <div v-if="props.summary" class="text-right">
          <span class="text-xs font-bold text-gray-400 uppercase tracking-widest">{{ t.dashboard.progress }}</span>
      ...
      <div class="relative z-10">
        <div class="flex items-baseline gap-2">
          ...
          <span class="text-gray-400 font-medium">/ {{ props.summary !== null ? formatDecimalHours(props.summary.expectedHours) : '--' }}</span>
        </div>
        <p class="text-sm font-bold uppercase tracking-wider text-gray-400 mt-1">{{ t.dashboard.workedHoursQuarter }}</p>
        
        <div class="mt-4 w-full bg-gray-100 dark:bg-gray-800 h-2 rounded-full overflow-hidden">
          <div 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="relative z-10">
        ...
        <p class="text-sm font-bold uppercase tracking-wider text-gray-400 mt-1">{{ t.dashboard.unjustifiedAbsences }}</p>
        <div v-if="(props.summary?.pendingJustifications || 0) > 0" class="mt-2 flex items-center gap-1 text-amber-600 dark:text-amber-400">
          <AlertCircle :size="12" />
          <span class="text-xs font-bold">{{ props.summary?.pendingJustifications }} {{ t.dashboard.pending }}</span>
        </div>
      </div>
      </div>
      ...
      <div class="relative z-10">
        ...
        <div v-if="props.summary" class="mt-2 flex items-center gap-1">
          <span class="text-xs font-bold uppercase text-gray-400">{{ t.dashboard.monthly }}:</span>
          <span class="text-xs font-bold tabular-nums" :class="monthlyBalanceClass">{{ formatDecimalHours(props.summary.monthlyBalance) }}</span>
        </div>
      </div>
      </div>
      ...
      <div class="relative z-10">
        ...
        <p class="text-sm font-bold uppercase tracking-wider text-gray-400 mt-1">{{ t.dashboard.vacationDays }}</p>
      </div>
      </div>
    </div>
  </div>
