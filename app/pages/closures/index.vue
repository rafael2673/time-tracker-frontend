...
import { Loader2, Lock, CalendarDays, FileText, CheckCircle2 } from 'lucide-vue-next'
  ...
  <div class="flex flex-col gap-6 max-w-6xl">
    <div class="flex flex-col md:flex-row justify-between md:items-end gap-4">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="w-40"><BaseSelect v-model="selectedMonth" :options="monthOptions" /></div>

        <button v-if="!closuresStore.isClosed && !closuresStore.isLoading" @click="isConfirmModalOpen = true" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-xl shadow-md transition-all flex items-center gap-2 cursor-pointer hover:scale-105 active:scale-95 h-[42px]">
          <Lock :size="16" />
          {{ t.closures.closeMonth }}
        </button>
        <div v-else-if="closuresStore.isClosed" class="px-4 py-2.5 bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 font-bold text-sm rounded-xl flex items-center gap-2 h-[42px] border border-emerald-100 dark:border-emerald-800/50">
      ...
      <div v-else class="w-full overflow-x-auto">
        <table class="w-full text-left border-collapse whitespace-nowrap">
          ...
          <tr class="border-b border-gray-100 dark:border-gray-800">
            <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ t.closures.employee }}</th>
            ...
            <th class="px-6 py-4 text-xs font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 text-right">{{ t.closures.banked }}</th>
          ...
          <tr v-for="c in closuresStore.closures" :key="c.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
            <td class="px-6 py-4 font-bold text-gray-900 dark:text-white">{{ c.employeeName }}</td>
            ...
            <td class="px-6 py-4 text-right font-bold text-indigo-600 dark:text-indigo-400">{{ formatDecimalHours(c.bankedHoursDelta) }}</td>
